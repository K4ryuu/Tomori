...
public Action ExecuteStringCommand(int client, char sCommandString[512])
{
	if (EMP_IsValidClient(client) && gH_Cvar_Tomori_Status_Enabled.BoolValue)
	{
		...
		if (StrContains(sMessage, "status ") == 0 || StrEqual(sMessage, "status", false))
		{
			...
			PrintToConsole(client, "#");
			Format(buffer, sizeof(buffer), "#	Server name: 	{hostname}");
			EMP_ReplaceFormats(buffer, sizeof(buffer));
			PrintToConsole(client, buffer);
			Format(buffer, sizeof(buffer), "#	Server IP: 	{server_ip}");
			EMP_ReplaceFormats(buffer, sizeof(buffer));
			PrintToConsole(client, buffer);
			Format(buffer, sizeof(buffer), "#	Player count: 	{player_count}({connecting_players})/{maxplayers}");
			EMP_ReplaceFormats(buffer, sizeof(buffer));
			PrintToConsole(client, buffer);
			Format(buffer, sizeof(buffer), "#	Current date: 	{current_date}");
			EMP_ReplaceFormats(buffer, sizeof(buffer));
			PrintToConsole(client, buffer);
			Format(buffer, sizeof(buffer), "#	Current time: 	{current_time}");
			EMP_ReplaceFormats(buffer, sizeof(buffer));
			PrintToConsole(client, buffer);
			Format(buffer, sizeof(buffer), "#	Current map: 	{currentmap}");
			EMP_ReplaceFormats(buffer, sizeof(buffer));
			PrintToConsole(client, buffer);
			Format(buffer, sizeof(buffer), "#	Nextmap: 	{nextmap}");
			EMP_ReplaceFormats(buffer, sizeof(buffer));
			PrintToConsole(client, buffer);
			PrintToConsole(client, "#");
			...
			if (strlen(status_flag) > 0)
			{
				...
				if (Client_HasAdminFlags(client, istatus_flag))
				{
					for (int idx = 1; idx <= MaxClients; idx++)
					{
						...
						{
							Format(buffer, sizeof(buffer), "# {userid}	    {playername} 		  {steam32}	  {connection_time}            {client_ip}");
							EMP_ReplaceFormats(buffer, sizeof(buffer), idx);
							PrintToConsole(client, buffer);
						}
					}
				}
				...
				{
					Format(buffer, sizeof(buffer), "# {userid}	    {playername} 		  {steam32}	  {connection_time}            {client_ip}");
					EMP_ReplaceFormats(buffer, sizeof(buffer), client);
					PrintToConsole(client, buffer);
				}
			}
			else
